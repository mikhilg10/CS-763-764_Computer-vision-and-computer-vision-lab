...
def geometric_loss(pred, target, range1, sigma):

  # Azimuth is between [0,360), Elevation is between (-90,90), In-plane Rotation is between [-180,180)

  C = pred.size(1)  # no. of classes/bins
  binSize = range1 // C  # size of each bin
  k_gt = target // binSize  # ground-truth bin index
  batch_size = pred.shape[0]
  Loss = torch.zeros(batch_size)
  for b in range(batch_size):
    for k,q in enumerate(pred[b]): # k: bin index; q:probability of each bin
        a = -1*(k_gt[b]-(k+1))/sigma
        Loss[b] += torch.exp(a)*torch.log(q)
  Loss *= (-1/C)
  
  return torch.mean(Loss)

class GeometricLoss(nn.Module):
    def __init__(self, range, sigma = 3):
        super(GeometricLoss, self).__init__()
        self.__range__ = range
        self.__sigma__ = sigma

    def forward(self, pred, target):
        return geometric_loss(pred, target,self.__range__,self.__sigma__)
